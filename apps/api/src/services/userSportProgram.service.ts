...
export class UserSportProgramService {
	...
	async getUserSportProgramById(id: string): Promise<UserSportProgram | null> {
	...
	async updateUserSportProgram(
		...
		if (userSportProgramToUpdate) {
			...
			return this.userSportProgramRepository.save(userSportProgramToUpdate);
		}
		return null;
	}
	async deleteUserSportProgram(id: string): Promise<boolean> {
		...
		if (userSportProgramToDelete) {
			await this.userSportProgramRepository.remove(userSportProgramToDelete);
			return true;
		}
		return false;
	}
