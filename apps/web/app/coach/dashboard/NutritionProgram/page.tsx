...
const Page: React.FC = () => {
	...
	>([]);
	const [mealsByProgram, setMealsByProgram] = useState<{
		[key: number]: MealType[];
	}>({});
			...
			for (const program of nutritionPrograms) {
				try {
					const response = await fetch(
						`${urlbase}/meal/nutritionProgram/${program.id}`,
					);
					if (!response.ok) {
						console.error(
							`Erreur lors de la récupération des meals pour le programme ${program.id}`,
						);
						continue;
					}
					const mealData: MealType[] = await response.json();
							...
							nutritionPrograms.map((program) => (
								<React.Fragment key={program.id}>
									<tr className="bg-white border-b hover:bg-gray-100 transition-colors">
										...
										{(mealsByProgram[program.id]?.length ??
										0 > 0) ? (
											mealsByProgram[program.id]?.map(
												(meal, index) => (
													<React.Fragment
														key={meal.id}
													>
														{index === 0 && (
															<>
																...
																<td className="px-4 py-2 text-center align-middle">
																	{meal.dayTime.toLocaleTimeString()}
																</td>
															</>
														)}
													</React.Fragment>
